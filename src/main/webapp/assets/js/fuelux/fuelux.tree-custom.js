...
(function (factory) {
	...
	var Tree = function (element, options) {
		...
		//this.$element.on('click.fu.tree', '.tree-branch-header:not(> .tree-branch-name > .ace-icon:first-child)', $.proxy( function(ev) { this.selectItem(ev.currentTarget); }, this));
		this.$element.on('click.fu.tree', '.tree-branch-header', $.proxy( function(ev) { this.selectItem(ev.currentTarget); }, this));
		/* EVAN */
		this.$element.on('click.fu.tree', '.tree-branch-header > .tree-branch-name > .ace-icon:first-child', $.proxy( function(ev) {
	...
	Tree.prototype = {
		...
		closeFolder: function (el) {
			...
			// manipulate branch/folder
			var eventType, classToTarget, classToAdd;
			if($el.find('.'+$.trim(this.options['open-icon'].replace(/(\s+)/g, '.')))) {
				eventType = 'closed';
		...
		toggleFolder: function (el) {
			var $el = $(el); // tree-branch-name
			if (!$el.hasClass('tree-branch-name'))
				$el = $el.closest('.tree-branch-name');
		...
		refreshFolder: function(folderId) {
			// toggle close
			this.closeFolder(folderId);
			$(folderId).find('.tree-branch-children').children().remove();
			this.openFolder(folderId); // open again
		},
		...
		/* EVAN */
		allLoadedItems: function() {
			var $branch = this.$element.find('.tree-branch:not(.hide)');
			var $items = this.$element.find('.tree-item:not(.hide)');
